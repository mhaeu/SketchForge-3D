...
describe("loft end rotation", () => {
  // The visible facets of a low-segment end must travel with that end's own
  // rotation. A corner-less end (Oval) borrows its sample angles from the other
  // end's corners, so without carrying the rotation difference its outline would
  // turn while its corners stayed put - the outline appeared to change shape
  // rather than spin.
  const ringAtTop = (topRotation: number) => {
    const geometry = createLoftGeometry({
      width: 20, depth: 10, height: 20, topWidth: 20, topDepth: 10,
      bottomShape: "Rectangle", topShape: "Oval",
      bottomRotation: 0, topRotation, segments: 8, layers: 2,
    });
    const pos = geometry.getAttribute("position").array as ArrayLike<number>;
    const count = pos.length / 3;
    let maxY = -Infinity;
    for (let i = 0; i < count; i += 1) maxY = Math.max(maxY, pos[i * 3 + 1]);
    const ring: Array<[number, number]> = [];
    for (let i = 0; i < count; i += 1) {
      if (Math.abs(pos[i * 3 + 1] - maxY) > 1e-6) continue;
      const x = pos[i * 3];
      const z = pos[i * 3 + 2];
      if (Math.hypot(x, z) < 1e-6) continue; // cap centre
      if (ring.some(([rx, rz]) => Math.hypot(rx - x, rz - z) < 1e-6)) continue;
      ring.push([x, z]);
    }
    return ring;
  };

  it("rotates a corner-less end rigidly instead of sliding its samples", () => {
    const degrees = 45;
    const radians = (degrees * Math.PI) / 180;
    const before = ringAtTop(0);
    const after = ringAtTop(degrees);
    ...
    for (const [x, z] of before) {
      ...
      const hit = after.some(([ax, az]) => Math.hypot(ax - rx, az - rz) < 1e-3);
      expect(hit, `rotated point ${rx.toFixed(3)},${rz.toFixed(3)} missing from the turned ring`).toBe(true);
    }
  });
});
